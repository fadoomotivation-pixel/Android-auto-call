...
// Statuses that mean the lead was genuinely driven forward — worth learning from.
const WON = ["site_visit", "negotiation", "proposal", "token_paid", "booked"];
...
const WIN_SYSTEM =
  "You are a real-estate sales trainer studying a WON lead (the rep moved it to a site visit or booking). " +
  "The material may be in Hindi, English or Hinglish — understand all three. From the calls, voice notes and " +
  "notes, extract the concrete moves that ACTUALLY worked to advance this lead — the lines said, the objection " +
  "handled, the offer used, the timing. " +
  'Reply ONLY as JSON {"lesson": string}. lesson = a short reusable playbook note (max ~80 words) in easy Roman ' +
  "Hinglish (aap-form) that ANOTHER telecaller could copy on a similar lead: what to say and do to get interested " +
  "→ site visit → booking. Be specific (quote the actual winning line/offer). If nothing clearly worked, reply " +
  '{"lesson": ""}. NEVER invent details that aren\'t in the material.';
...
async function harvestOne(admin: SupabaseClient, contactId: string): Promise<boolean> {
  ...
  if (!c?.company_id || !WON.includes(String(c.status))) return false;
  ...
  try {
    ...
    const { error } = await admin.from("knowledge_chunks").insert({
      company_id: c.company_id, source_kind: "win", source_id: sourceId,
...
Deno.serve(async (req) => {
  ...
  if (bearer === SERVICE) {
    ...
    const { data: existing } = await admin.from("knowledge_chunks")
      .select("source_id").eq("source_kind", "win")
